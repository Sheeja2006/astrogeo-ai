...
export default function SpaceBackground() {
  ...
  useEffect(() => {
    ...
    function initPlanets() {
      planets.length = 0;
      const defs = [
        // { x%, y%, radius, color, rings: bool, moons: int, speed }
        ...
        { xp: 0.5,  yp: 0.08, r: 10, color: '#ef9f27', rings: false, moons: 1, speed: 0.0007 },
      ];
      defs.forEach(d => {
        const moons = [];
    ...
    function drawShootingStars() {
      for (let i = shootingStars.length - 1; i >= 0; i--) {
        ...
        grad.addColorStop(1, `rgba(220,200,255,${s.alpha})`);
        ctx.beginPath();
    ...
    function drawEarth(time) {
      // Floating Earth bottom-left corner
      const x = width * 0.06;
      const y = height * 0.88;
      const r = 34;
    ...
    window.addEventListener('resize', () => { resize(); initStars(); initPlanets(); });
